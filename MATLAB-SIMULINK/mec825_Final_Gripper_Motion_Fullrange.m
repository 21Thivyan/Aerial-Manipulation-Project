...
%theta2 = 29.2:2:89.2; % Driving Link Motion Range
theta2 = 0:2:360; % Driving Link Motion Range
...
for i = 1:length(theta2)
    ...
    % Velocity Analysis (Angular Velocities)
    r = [ L2*cosd(theta3), -L3*cosd(theta4); 
          L2*sind(theta3), -L3*sind(theta4) ];
    
    v = [ -L1 * sind(theta2(i)); -L1 * cosd(theta2(i))];

    w = r \ v; % Solve for angular velocities
    om3(i) = w(1);
    om4(i) = w(2);
    ...
    % Calculate Distance Between P2 and Mirrored P2
    P2_dist = norm(P2_rot - P2_mirror);
    P2_distance = [P2_distance, P2_dist];
...
% Plot Distance Between P2 and Mirrored P2
figure(2);
plot(theta2, P2_distance, 'b', 'LineWidth', 2);
xlabel('\theta_2 (degrees)');
ylabel('Distance Between P2 and Mirrored P2 (cm)');
title('Distance Variation of P2 and Mirrored P2');
grid on;


% Angular Velocity of θ₃
figure(3);
plot(theta2, om3, 'LineWidth', 2);
